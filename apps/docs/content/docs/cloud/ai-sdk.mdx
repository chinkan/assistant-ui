...
export default function Chat() {
  // Thread management included — auto-creates anonymous cloud from env var
...
Standalone thread list management. Can be used independently or passed to `useCloudChat({ threads })` for separate control over thread state.
