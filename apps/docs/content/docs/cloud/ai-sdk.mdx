...
export default function Chat() {
  ...
  // For custom config, pass: { cloud, threads: { includeArchived, autoGenerateTitle }, onSyncError }
...
**3. Thread config** — Configure thread behavior:

```tsx
const chat = useCloudChat({
  threads: {
    includeArchived: true,      // Show archived threads
    autoGenerateTitle: false,   // Disable auto-title
  },
  onSyncError: (err) => toast.error(err.message),
});
```

**4. External thread management** — When threads need to be accessed from a separate component:
...
| `options.threads` | `UseThreadsResult \| ThreadsConfig` | Thread configuration or external thread management. Pass `UseThreadsResult` from `useThreads()` when you need thread operations in a separate component, or pass `ThreadsConfig` to configure internal thread management |
| `options.threads.includeArchived` | `boolean` | Include archived threads (default: `false`) |
| `options.threads.autoGenerateTitle` | `boolean` | Auto-generate title after first response on new threads (default: `true`) |
